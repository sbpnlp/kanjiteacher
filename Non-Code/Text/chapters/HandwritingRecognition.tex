...
\emph{Hànzì} characters (\cjk{汉字}) as they were imported into the Japanese 
...
\paragraph{Deskewing}
\label{sec:deskewing}
\emph{Deskewing} is a technique that modifies the slant of an input. Deskewing 
...
\emph{Baseline drift correction} detects a baseline or horizontal line relative
...
\emph{Stroke length normalisation}, sometime called \emph{resampling} normalises
...
\begin{itemize}
  ...
  \item \textbf{Time / Zone Sequences} that code zones on the writing display
    (see~\ref{sec:timeandzones}).
  ...
  \item \textbf{HMM Analysis} is independent of the other methods used, 
    generally it performs a statistical analysis of the features used, 
...
the individual characters of the alphabet. For the Latin alphbet, the systems 
use features like descender or not decsender, dot or no dot 
...
one element the character \emph{f}. A HWR system can come to similar conlusions
by checking if a dot is present \emph{i, j}, if the character has an open line
\emph{c, u, v, w, y} or a closed line \emph{a, b, d, e, g, o, p, q}. Systems 
that perform feature analysis have the potential to use psycholinguistic 
...
\subsubsection{Time Sequences and Zones}
\label{sec:timeandzones}
