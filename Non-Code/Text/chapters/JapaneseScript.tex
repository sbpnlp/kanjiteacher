...
\gll 
 ...
 \cjk{見ました} 
 \cjk{か。} \\
 Maria: Yamada-San, [Firebird] say anime obj-particle already seen question-particle \\
\trans 'Maria: Ms Yamada, say, have you seen the Firebird cartoon yet?'
\end{exe}
In example~(\ref{mariaYamada}), taken from~\shortcite{Katsuki2006a}, the blending
of the different scripts can be seen:\\
Both the foreign name \emph{Maria} and the borrowing \emph{anime} (Jap. short for
'animation') are written in Katakana. Kanji are used for:
\begin{itemize}
\item The Japanese name \emph{Yamada} (\cjk{山田})
\item The nouns \emph{fire} (\cjk{火}) and \emph{bird} (\cjk{鳥})
\item The verb \emph{see} (\cjk{見})
\end{itemize}
The rest is written in Hiragana:
\begin{itemize}
  \item The politeness ending \emph{san} (\cjk{さん}) for addressing a person 
        with their name
  \item The genitive particle \emph{no} (\cjk{の}) between \emph{fire} (\cjk{火}
        and \emph{bird} (\cjk{鳥}) to yield \emph{firebird} (\cjk{火の鳥})
  \item The interjection \emph{say} (\cjk{とうい})
  \item The object particle \emph{wo} (\cjk{を})
  \item The adverb \emph{already} (\cjk{もう})
  \item The past tense konjugation of the verb (\cjk{ました})
  \item The question particle \emph{ka} (\cjk{か})
\end{itemize}
