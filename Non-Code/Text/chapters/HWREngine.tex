...
%   \(B := <H, s, s>\)
% \end{quote}
% The use of boxing is used multifunctional in the prototype. One of the uses is
...
% the space given to a smaller Kanji is still a square.
...
% %xxx think about a different type of scaling for strokes and characters
% %xxx or radicals.  that would call for a separate scaling section under
% %xxx character recognition.
...
% a certain threshhold %xxx which threshhold? say the number!
% that has been determined empirically, %xxx how?
...
% of their values. 
% The length \(l\) is calculated as the sum of the euclidian distances \(d_i\) 
...
% the euclidian distance between the original point \(P_{I,db},P_{E,db}\)
...
% sequence with respect to the regression line of the database point sequence.
...
% the point analised point sequence was identical with the stored point sequence.
...
%          match, if the follwoing equation is fulfilled \\
%          \begin{quote}
%          \(
...
% The point distance calculations are euclidian, ignoring the fact that the
...
% \)
% \end{quote}
% \(<x_i, y_i>\) are the regular 2-dimensional point cooredinates, 
...
% comparision with the regular DTW is reported in 
...
% dynamic time warping is used as a fallback option.
...
% handle point of the box. The euclidian distance between the \(P_{db}\) and 
...
% \)
% \end{quote}
% The matrix \(M\) forms the basis of the Radical recognition. The highest 
% confidence value for a Radical will be achieved if
% \begin{itemize}
%   \item \( m = n \) 
%   \item the highest matching value \(\mu_{i,j}\) for each stroke is above a
%         threshold in order to conclude that it matched with its corresponding 
%         stored stroke from the database.
%   \item the highest matching values \(\mu_{i,j}\) for all strokes form a diagonal 
%         path through the matrix, with \( i = j \) for each highest matching 
%         value, starting at \(\mu_{1,1}\), ending at 
%         \(\mu_{n,n}\) (and \(m=n\)).
% \end{itemize}
% If two identical stroke sequences are analised, the ideal is fulfilled.
...
In order to scale to the normalised size, the length of an edge of the bounding
box is compared to the length of the normalised character.
