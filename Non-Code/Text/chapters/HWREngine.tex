...
For the matching of a straight stroke there are are a number of features
...
\begin{itemize}
  ...
  \item Initial point
  \item Endpoint
  \item Total number of measured coordinate points
  \item Velocity with which the stroke was drawn
\end{itemize}
...
In order to math two straight strokes with each other, the vicinity of their 
start point and their end point is considered.
Furthermore, the total length, the number of points and the velocity are 
considered. A feature vector \(F\) for the description of a stroke then 
...
%xxx ausgleichsgerade hier, if applicabale.
