...
%   the exchange of the custom format with those)
...
recognition~\shortcite{Bhaskarabhatla2004}. 
...
input~\shortcite{UnipenConsortium2006}.
...
by~\shortciteANP{UnipenConsortium2006}~\citeyear{UnipenConsortium2006}. 
...
studied~\shortcite{Keshari2008}. These techniques are not necessary 
...
\label{sec:hwre:dataformatdescription}
  ...
  % if internal structure of a stroke is changed.
  % (unipen is only text based, inkml does not help here, but the system allows
  % exchange of the custom format with those)
...
The 
radical~\cjk{木}~(Jap.\ pron.\ \cjk{キ}/ki; Eng.\ 'tree'), 
...
character~\cjk{東}~(Jap.\ pron.\ \cjk{ヒガシ}/higashi; Eng.\ 'east') 
is shown.
It consists of the 
radicals~\cjk{日}~(Jap.\ pron.\ \cjk{ヒ}/hi; Eng.\ 'sun')
and~\cjk{木}~(Jap.\ pron.\ \cjk{キ}/ki; Eng.\ 'tree').
...
provided\footnote{KanjiDict2 is freely available from \url{http://www.csse.monash.edu.au/~jwb/Kanjidic2/}} in an XML text file~\shortcite{Breen2004}.

Listing~\ref{lst:lexiconentryEAST} shows a sample database entry of 
\shortciteANP{Breen2004}'s~\citeyear{Breen2004} 
...
character index in~\shortciteANP{Hadamitzky1995}'s~\citeyear{Hadamitzky1995} 
...
\newpage %this is not necessary, if the listing can be placed variably by latex because it is shorter than one page now
\begin{xmlcode}
...
\begin{quote}
\(
  S_c := \sum\limits_{i=1}^{n} \sqrt{(x_{i,db}-x_{i,exp})^2+(y_{i,db}-y_{i,exp})^2} % d_{i} 
\)
\end{quote}
...
%uses the angle of each line segment regarding the 
...
%   * if describing a solution to a problem: what alternatives were
%     there to solve it, why was this solution chosen? 
%     again - part of the structural recognition process that allows 
...
%     what made it the best choice? was it the optimal solution?
%     the other HWR approaches do not allow the partial structural recognition
