...
%xxx if you want to add a gfx of a stroke order there is one here:
%http://en.wiktionary.org/wiki/%E6%9D%B1
%however, the example is not great, since it is one of the rare cases,
%where the stroke sequence is 'garbled' in the way that one radical is started,
%then the next one is started and finished, then the first one is finished.
%maybe a totally different example would be better?
%what about the cedar \cjk{杉} from the japanese chapter?
%radical 75 - tree \cjk{木}+ radical 59 hair / bristle
%\cjk{彡} 
%or just leave it and show the stroke order as problematic?!
\begin{xmlcode}
  \begin{lstlisting}[emph={hLevel,hwTraces,label,alternative},
                     emphstyle=\color{blue}\textbf,
                     emph={[2]level,labelSrcRef,labelType,traceRef,from,to},
...
Notice xxx yyy the stroke order of EAST shown in figure \ref{fig:strokeorderof東}
\section{Database}
