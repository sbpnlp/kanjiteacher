...
function getPollHeight(search_poll_frame,search_iframe){
	if(document.height){
		var searchFrameHeightNum = parent.frames[search_iframe].document.height + 20;
		var iframeMinHeight = 420;
		if(searchFrameHeightNum<iframeMinHeight){
			document.getElementById(search_poll_frame).style.height = iframeMinHeight + "px";
		}else{
			document.getElementById(search_poll_frame).style.height = searchFrameHeightNum + "px";
		}
	}else{
